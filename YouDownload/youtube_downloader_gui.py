...
class YouTubeDownloaderGUI:
	...
	def setup_ui(self):
		...
		self.button_icons = {}
		try:
			# Resolve icons folder for both dev and bundled exe
			icons_dir = None
			for cand in [
	...
	def progress_hook(self, d):
		...
		if d['status'] == 'downloading':
			...
			mb_downloaded = 0
			mb_total = 0
			# total bytes attempts
			total_bytes = d.get('total_bytes') or d.get('total_bytes_estimate') or d.get('filesize')
			...
			if speed:
				...
				if total_bytes and total_bytes > 0:
					...
					status = f"Downloading: {mb_downloaded:.2f} MB - {speed_mb:.2f} MB/s"
			else:
				if total_bytes and total_bytes > 0:
					status = f"Downloading: {percent:.1f}% ({mb_downloaded:.2f} MB / {mb_total:.2f} MB)"
				else:
					status = f"Downloading: {mb_downloaded:.2f} MB"
